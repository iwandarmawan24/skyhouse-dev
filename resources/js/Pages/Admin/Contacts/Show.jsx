import AdminLayout from '@/Layouts/AdminLayout';
import { Link, router } from '@inertiajs/react';
import { Button } from '@/Components/ui/Button';
import { Badge } from '@/Components/ui/Badge';

export default function Show({ contact }) {
    const formatDate = (date) => {
        return new Date(date).toLocaleDateString('id-ID', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric',
            hour: '2-digit',
            minute: '2-digit',
        });
    };

    const handleStatusChange = (newStatus) => {
        router.patch(`/admin/contacts/${contact.id}/mark-read`, {
            status: newStatus,
        }, {
            preserveScroll: true,
        });
    };

    return (
        <AdminLayout>
            {/* Page Header */}
            <div className="mb-6">
                <div className="flex items-center gap-4 mb-2">
                    <Link href="/admin/contacts" className="text-gray-600 hover:text-gray-900">
                        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
                        </svg>
                    </Link>
                    <h1 className="text-2xl font-bold text-gray-900">Contact Details</h1>
                </div>
                <p className="text-gray-600 ml-10">View customer inquiry information</p>
            </div>

            {/* Contact Card */}
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                {/* Header */}
                <div className="bg-blue-600 px-6 py-4 flex items-center justify-between">
                    <h2 className="text-xl font-semibold text-white">Customer Information</h2>
                    <Badge variant={
                        contact.status === 'new' ? 'default' :
                        contact.status === 'in_progress' ? 'secondary' :
                        'success'
                    } className="text-white">
                        {contact.status === 'new' && 'New'}
                        {contact.status === 'in_progress' && 'In Progress'}
                        {contact.status === 'resolved' && 'Resolved'}
                    </Badge>
                </div>

                {/* Content */}
                <div className="p-6 space-y-6">
                    {/* Contact Info */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label className="block text-sm font-medium text-gray-500 mb-1">Full Name</label>
                            <p className="text-lg font-medium text-gray-900">{contact.full_name}</p>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-500 mb-1">Email</label>
                            <a
                                href={`mailto:${contact.email}`}
                                className="text-lg font-medium text-blue-600 hover:text-blue-700"
                            >
                                {contact.email}
                            </a>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-500 mb-1">Phone</label>
                            <a
                                href={`tel:${contact.phone}`}
                                className="text-lg font-medium text-blue-600 hover:text-blue-700"
                            >
                                {contact.phone}
                            </a>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-500 mb-1">Current Residence</label>
                            <p className="text-lg text-gray-900">{contact.residence}</p>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-500 mb-1">Subject</label>
                            <p className="text-lg text-gray-900">
                                {contact.subject === 'inquiry' && 'General Inquiry'}
                                {contact.subject === 'purchase' && 'Purchase Information'}
                                {contact.subject === 'visit' && 'Schedule Visit'}
                                {contact.subject === 'other' && 'Other'}
                            </p>
                        </div>

                        <div>
                            <label className="block text-sm font-medium text-gray-500 mb-1">Project Interest</label>
                            <p className="text-lg text-gray-900">
                                {contact.project === 'kinary' ? 'Kinary House' : 'Other Projects'}
                            </p>
                        </div>

                        <div className="md:col-span-2">
                            <label className="block text-sm font-medium text-gray-500 mb-1">Date Submitted</label>
                            <p className="text-lg text-gray-900">{formatDate(contact.created_at)}</p>
                        </div>
                    </div>

                    {/* Message */}
                    <div className="border-t pt-6">
                        <label className="block text-sm font-medium text-gray-500 mb-3">Message</label>
                        <div className="p-4 bg-gray-50 rounded-lg">
                            <p className="text-gray-900 whitespace-pre-wrap">{contact.message}</p>
                        </div>
                    </div>

                    {/* Status Management */}
                    <div className="border-t pt-6">
                        <label className="block text-sm font-medium text-gray-500 mb-3">Update Status</label>
                        <div className="flex items-center gap-3">
                            <Button
                                onClick={() => handleStatusChange('new')}
                                variant={contact.status === 'new' ? 'default' : 'outline'}
                                size="sm"
                            >
                                New
                            </Button>
                            <Button
                                onClick={() => handleStatusChange('in_progress')}
                                variant={contact.status === 'in_progress' ? 'default' : 'outline'}
                                size="sm"
                            >
                                In Progress
                            </Button>
                            <Button
                                onClick={() => handleStatusChange('resolved')}
                                variant={contact.status === 'resolved' ? 'default' : 'outline'}
                                size="sm"
                            >
                                Resolved
                            </Button>
                        </div>
                    </div>

                    {/* Action Buttons */}
                    <div className="border-t pt-6 flex items-center gap-4">
                        <a
                            href={`mailto:${contact.email}?subject=Re: Your ${contact.subject} inquiry`}
                            className="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg transition"
                        >
                            <svg
                                className="w-5 h-5 mr-2"
                                fill="none"
                                stroke="currentColor"
                                viewBox="0 0 24 24"
                            >
                                <path
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                                    strokeWidth={2}
                                    d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
                                />
                            </svg>
                            Reply via Email
                        </a>

                        <a
                            href={`https://wa.me/${contact.phone.replace(/\D/g, '')}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white font-semibold rounded-lg transition"
                        >
                            <svg className="w-5 h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                            </svg>
                            Contact via WhatsApp
                        </a>
                    </div>
                </div>
            </div>
        </AdminLayout>
    );
}
